...
params = dict(vmax=0.7, vmin=0, vin=0, step=1e-12, sim_time=5e-9, wl_pw=2e-9, wl_del=0.5e-9,\
               rise=0.1e-9, fall=0.1e-9, tp=5e-9, bl_cap=2.5e-13)
read_spice_netlist(fname='sram8T_read', params=params)

sys('hspice sram8T_read_post.sp -o s1')
...
lis_vn1 = lis_M[:,3]
lis_vn2 = lis_M[:,4]
print(f"PW: {params['wl_pw']}, DVBL: {lis_vrbl[0]-lis_vrbl[-1]}V")
...
ax1.plot(lis_t*1e9, lis_vrwl, linewidth=4, color='r')
ax2.plot(lis_t*1e9, lis_vrbl, linewidth=4, color='b')
ax3.plot(lis_t*1e9, lis_vn1, linewidth=4, color='r')
ax3.plot(lis_t*1e9, lis_vn2, linewidth=4, color='r')
ax1.set(ylabel='$RWL(V)$')
ax1.set(ylim=[-0.1, 0.8])
ax1.set(yticks=[0, 0.7])
ax2.set(ylabel='$RBL(V)$')
